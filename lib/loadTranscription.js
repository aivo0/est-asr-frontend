const loadHtml = text => {
  ...
  const mapWordsToLinks = word => {
    ...
    if (word.confidence && word.confidence < 0.9)
      obj.attributes.confidence =
        Math.round(word.confidence * 100) + "% kindlust";
    // Apply color if low confidence
    if (word.confidence < 0.9 && word.confidence >= 0.7)
      obj.attributes.background = "#cce8cc";
    if (word.confidence < 0.7) obj.attributes.background = "#ff6";
    return [obj, { insert: " " }];
  };
